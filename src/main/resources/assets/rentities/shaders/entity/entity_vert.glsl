...
// Per-instance animation phases generated by entity_cull.comp. The buffer is only
// consumed on the indirect/culling path; the CPU-draw fallback keeps the legacy
// expressions below so feature failure degrades to correctness, not missing animation.
layout(std430, binding = 17) readonly buffer FastAnimationFrameBuffer {
    vec4 fastAnimationFrames[];
};

uniform int uFastAnimation;
...
int getAnimationLod(vec3 entityPos) {
    if (uAnimationLodEnabled == 0) return 0;
    // Instance positions are camera-relative render coordinates, so the camera is the origin.
    float d2 = dot(entityPos, entityPos);
    float medium2 = uAnimationLodMediumDistance * uAnimationLodMediumDistance;
    float far2 = uAnimationLodFarDistance * uAnimationLodFarDistance;
    if (d2 >= far2) return 2;
    if (d2 >= medium2) return 1;
    return 0;
}

vec4 getFastAnimationFrame(int instanceID) {
    return fastAnimationFrames[instanceID];
}
...
// These are populated once per vertex from the per-instance frame. They replace
// repeated walk-cycle trig calls with cheap scalar reuse.
vec4 gFastAnim = vec4(0.0);
int gAnimationLod = 0;
...
mat4 getQuadBone(int b, EntityInstance inst) {
    ...
    float phaseSin =
        phase == 0.0 ? gFastAnim.x :
        phase == PI ? -gFastAnim.x :
        phase == PI * 0.5 ? gFastAnim.y :
        -gFastAnim.y;
...
mat4 getHorseBone(int b, EntityInstance inst) {
    ...
    float lr = (phase == 0.0 ? gFastAnim.z : -gFastAnim.z) * sa * 1.4;
...
mat4 getBirdBone(int b, EntityInstance inst) {
    ...
    if (b == BONE_HEAD)
        return pivotRot(
            p,
            rotX(gFastAnim.x * sa * 0.5 + inst.headPitch));
...
mat4 getInsectBone(int b, EntityInstance inst) {
    ...
    return pivotRot(
        ...
        rotX(
            ...
            inst.limbSwingAmount * gAnimationAmountScale *
            0.6));
}
...
mat4 getAquaticLegsBone(int b, EntityInstance inst) {
    ...
    return pivotRot(
        p,
        rotX((ph == 0.0 ? gFastAnim.x : -gFastAnim.x) * sa));
}
...
mat4 getFrogBone(int b, EntityInstance inst) {
    ...
    return pivotRot(
        ...
        rotX(
            -crouch * 1.2 +
            (ph == 0.0 ? gFastAnim.x : -gFastAnim.x) * sa * 0.8) *
        rotZ(crouch * 0.5));
}
...
void main() {
    ...
    gAnimationLod = getAnimationLod(vec3(inst.posX, inst.posY, inst.posZ));
    gAnimationAmountScale = (gAnimationLod == 1)
        ? clamp(uAnimationLodMediumScale, 0.0, 1.0)
        : 1.0;

    if (uFastAnimation != 0) {
        gFastAnim = getFastAnimationFrame(instanceID);
    } else {
        float walk0662 = inst.limbSwing * 0.6662;
        float walk08 = inst.limbSwing * 0.8;
        gFastAnim = vec4(sin(walk0662), cos(walk0662), sin(walk08), cos(walk08));
    }
